...
class InfoAgent:
    ...
    def __init__(self, api_url=DATABASE_URL, model_name=DEFAULT_MODEL):
        """
        Initialize the info agent with API URL and LLM model
        
        Args:
            api_url: Base URL for the product search API
            model_name: Name of the Ollama model to use
        """
        logger.info(f"InfoAgent.__init__: Initializing with api_url={api_url}, model_name={model_name}")
    ...
    def process_query(self, query: str, client_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            llm_response = self.llm.generate(classification_prompt).strip().lower()
            ...
            logger.error(f"Error during LLM query processing: {e}")
