...
from chatbot.agents.agent_manager import AgentManager
from utils.TTS_Piper import TTS_Piper
...
async def process_text_task(text: str, client_id: str):
    ...
    if text_response:
        ...
        # Send generating audio event
        await send_event(client_id, EventType.GENERATING_AUDIO)
        
        # Allow event loop to process the event
        await asyncio.sleep(0)
        
        # Generate TTS audio in a non-blocking way
        file_audio = await asyncio.get_event_loop().run_in_executor(
            None, lambda: tts.run_tts(text_response)
        )
        
        # Read audio file in a non-blocking way
        audio_content = await asyncio.get_event_loop().run_in_executor(
            None, lambda: open(file_audio, "rb").read()
        )
        
        # Send audio response
        await send_message(client_id, "audio_response", base64.b64encode(audio_content).decode("utf-8"))
