...
from chatbot.OllamaModel import OllamaModel
...
class IngredientBasedRecipeAgent:
    def __init__(self, api_url=DATABASE_URL, model_name=DEFAULT_MODEL):
        """
        Initialize the ingredient-based recipe agent with API URL and LLM model
        
        Args:
            api_url: Base URL for the product search API
            model_name: Name of the Ollama model to use
        """
        logger.info(f"IngredientBasedRecipeAgent.__init__: Initializing with api_url={api_url}, model_name={model_name}")
        ...
        for name in ingredient_names:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"API error for ingredient '{name}': {response.status_code}")
                    product_ids.append(None)  # API error
            except requests.exceptions.RequestException as e:
                logger.error(f"API connection error for ingredient '{name}': {str(e)}")
