...
def get_store_products(store_id: str):
    ...
    locations_response = supabase.table("locations").select("*").eq("store_id", store_id).execute()
    store_locations = locations_response.data
    
    results = []
    for location in store_locations:
        product_response = supabase.table("products").select("*").eq("id", location["product_id"]).execute()
...
def search_recipes(query: SearchQuery):
    ...
    # Log the advanced recipe search
